...
--Thêm bảng OTP_XACNHAN để lưu mã OTP, phục vụ cho việc login của admin
CREATE TABLE OTP_XACNHAN (
    Username NVARCHAR(50) NULL,         -- Tài khoản xác nhận
    Email NVARCHAR(100) NULL,           -- Email để gửi OTP
    MaOTP NVARCHAR(6) NULL,             -- Mã OTP 6 chữ số
    ThoiGianTao DATETIME DEFAULT GETDATE(), -- Thời gian tạo mã
    TrangThai NVARCHAR(20) DEFAULT N'Chưa dùng' -- Trạng thái: 'Chưa dùng' | 'Đã dùng'
);
GO
...
BEGIN
    ...
    DECLARE @username NVARCHAR(50) = (SELECT Username FROM TKNHANVIEN WHERE TrangThai = N'Đang đăng nhập' );
...
--trigger xóa mã otp khi tạo mã otp mới
CREATE TRIGGER trg_Delete_OTP_HetHan
ON OTP_XACNHAN
AFTER INSERT
AS
BEGIN
